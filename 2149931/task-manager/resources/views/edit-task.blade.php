    ...
    <div class="container px-lg-5">
        <div class="p-4 p-lg-5 bg-light rounded-3 text-center">
            <div class="m-4 m-lg-5">
                ...
                <br>
                <form action="{{route('edit.task', $task->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                <!-- Text input -->
                    <div class="form-outline mb-4">
                        <label class="form-label" for="form6Example3">Title</label>
                        <input type="text" id="form6Example3" class="form-control" name="title"  value="{{$task->title}}" required/>
                    </div>
                    ...
                    <div class="form-outline mb-4">
                        ...
                        <select class="form-control" name="category">
                            @foreach($categories as $category)
                                <option value="{{$category->id}}" @if($category->id == $task->id) selected @endif>{{$category->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-outline mb-4">
                        ...
                        <textarea class="form-control" id="form6Example7" name="description" rows="4">{{$task->description}}</textarea>
                    </div>
